...
export class OrderComponent implements OnInit {
  ...
  totalCount: number = 0;
  totalDelivery: number = config.deliveryAmount;
    ...
    this.cartService.getCart()
      .subscribe((data: CartType | DefaultResponseType): void => {
        ...
        if (!this.cart || (this.cart && this.cart.items.length === 0)) {
          this._snackBar.open('Товары в корзине отсутствуют, там пока нечего делать =)')
  ...
  changeDeliveryType(type: DeliveryType): void {
    ...
    this.totalDelivery = type === DeliveryType.delivery ? config.deliveryAmount : 0;
    this.updateDeliveryTypeValidation();
  ...
  createOrder() {
    if (this.orderForm.valid && this.orderForm.value.firstName && this.orderForm.value.lastName &&
      this.orderForm.value.email && this.orderForm.value.phone && this.orderForm.value.email &&
      ...
      this.orderService.createOrder(paramsObject).subscribe({
        ...
        error: (errorResponse: HttpErrorResponse) => {
          if (errorResponse.error && errorResponse.error.message) {
            ...
            this._snackBar.open('Произошла ошибка при обработке заказа.')
          }
        }
      });
    } else {
      this._snackBar.open('Для выполнения заказа необходимо заполнить форму.');
