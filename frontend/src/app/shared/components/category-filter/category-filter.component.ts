...
export class CategoryFilterComponent implements OnInit {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (this.type) {
        ...
        if (this.categoryWithTypes && this.categoryWithTypes.types && this.categoryWithTypes.types.length > 0) {
          const hasActiveType = this.categoryWithTypes.types.some(type =>
  ...
  updateFilterParamFromTo(param: string, value: string): void {
    if (param === CONSTANTS.heightTo || param === CONSTANTS.heightFrom || param === CONSTANTS.diameterTo || param === CONSTANTS.diameterFrom) {
